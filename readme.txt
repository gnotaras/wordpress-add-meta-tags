...
Add-Meta-Tags allows filtering of the generated metatags through the following filters:

1. `amt_basic_metatags`
1. `amt_opengraph_metatags`
1. `amt_dublin_core_metatags`

The function that is hooked to the filter must accept one argument, which is an array containing the relevant metatags, and return it after processing it.

*Example scenario*: you want to replace the autogenerated `og:site_name` Opengraph metatag with a custom one.
