...
Add-Meta-Tags generates metadata that is used in the head area of the HTML page or embeded in the body (wrapped around the content or in the footer area).
...
1. `amt_schemaorg_metadata_content` - applied to the Schema.org Microdata that has been generated by Add-Meta-Tags and is embeded around the content. The hooked function should accept and return 1 argument: an array of microdata. Note that `articleBody` is added to the array after filtering.
