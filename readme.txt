...
Add-Meta-Tags allows filtering of the generated metatags and also of some core functionality through filters. This way advanced customization of the plugin is possible. The available filters are:

1. `amt_metatags` - applied to the all metatags that have been generated by Add-Meta-Tags. The hooked function should accept and return 1 argument: an array of meta tags.
1. `amt_basic_metatags` - applied to the basic metatags (description, keywords, etc) that have been generated by Add-Meta-Tags. The hooked function should accept and return 1 argument: an array of meta tags.
1. `amt_opengraph_metatags` - applied to the OpenGraph metatags that have been generated by Add-Meta-Tags. The hooked function should accept and return 1 argument: an array of meta tags.
1. `amt_twitter_metatags` - applied to the Twitter Cards metatags that have been generated by Add-Meta-Tags. The hooked function should accept and return 1 argument: an array of meta tags.
1. `amt_dublin_core_metatags` - applied to the Dublin Core metatags that have been generated by Add-Meta-Tags. The hooked function should accept and return 1 argument: an array of meta tags.
1. `amt_googleplus_metatags` - applied to the 'author' and 'publisher' links that have been generated by Add-Meta-Tags for Google+. The hooked function should accept and return 1 argument: an array of links.
1. `amt_schemaorg_microdata` - applied to the Schema.org Microdata that has been generated by Add-Meta-Tags. The hooked function should accept and return 1 argument: an array of microdata. Note that `articleBody` is added to the array after filtering.
...
*Example scenario 1*: you want to replace the autogenerated `og:site_name` Opengraph metatag with a custom one.

This can easily be done by hooking a custom function to the `amt_opengraph_metatags` filter:
...
add_filter( 'amt_opengraph_metatags', 'customize_og_sitename_metatag', 10, 1 );
`
This code can be placed inside your theme's `functions.php` file.

*Example scenario 2*: you want to limit the generation of metadata by Add-Meta-Tags to specific post types.
...
1. `amt_metadata()` : prints the full metadata as generated by Add-Meta-Tags.
