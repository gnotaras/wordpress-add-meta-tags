...
- [2.10.11](http://www.codetrax.org/versions/349)
 - Improved the function that cleans up the post content before a description is generated by utilizing the `wp_strip_all_tags()` function instead of `wp_kses()` and added an extra filter hook, which allows the preprocessing of the post content after the shortcodes have been expanded but before the HTML tags are stripped. Thanks to <em>Its-Thomas</em> for valuable feedback!
